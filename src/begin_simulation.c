...
/*   Updated: 2023/12/15 08:55:58 by smatthes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	join_threads(t_main_data *main_data, int other_threads,
				int num_philo_thread, int num_monitoring_thread);
...
int	begin_simulation(t_main_data *main_data)
{
	...
	join_threads(main_data, 2, main_data->num_philo, main_data->num_philo);
	return (0);
}
...
static int	create_threads(t_main_data *main_data)
{
	...
	if (main_data->min_times_eat >= 0)
		if (pthread_create(&main_data->eat_count_thread, NULL,
			...
			return (join_threads(main_data, 1, 0, 0));
	i = 0;
	while (i < main_data->num_philo)
	{
		if (pthread_create(main_data->philo_threads + i, NULL, &philo_routine,
				main_data->philos + i) != 0)
			return (join_threads(main_data, 2, i, i));
		if (pthread_create(main_data->philo_monitor_threads + i, NULL,
				&philo_monitor_routine, main_data->philos + i) != 0)
			return (join_threads(main_data, 2, i, i - 1));
		i++;
	}
	return (1);
}
		...
		int num_philo_thread, int num_monitoring_thread)
{
	...
	if (other_threads >= 2 && main_data->min_times_eat >= 0)
